...
#####
# need a dataframe that shows types of transfers out
...
# loop to create plots for stlmt school corp circle markers ####
...
for (i in 1:length(schl_corps)){
  ...
  ggsave(filename = paste0("plots/stlmt_plots_new/", xfr$stlmt_corp_id[xfr$stlmt_corp_name == schl_corps[i]][1], "-", xfr$stlmt_corp_id[xfr$stlmt_corp_name == schl_corps[i]][1], ".png"),
...
plots <- unique(list.files("plots/stlmt_plots_new"))
...
for(i in 1:length(plots)){
  ...
  put_object(
    ...
    bucket = bucket,
    file = paste0("plots/stlmt_plots_new/", plots[i]),
    region = region,
