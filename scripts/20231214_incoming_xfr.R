...
# script creates chart that shows the annual number of students transferring into a given school
# from any school corporation that sends transfer students from 2018 - 2023; also 
# contrasts the most recent available (2023) test state and SAT test scores between the enrolled
# school and the school corporation sending the transfer student(s). Not all schools have test
# data available.
...
ixfr <- read_csv("clean_data/transfers_18_23_ll.csv") |> 
...
# some transfer relationships between a school corporation and an enrolled school only exist in one year. In those instances, geom_line doesn't fit and geom_point should be used instead. if else statement checks if a school corp/enrolled school relationship exists for more than one year (if more than 1 row exists) and assigns a plot with geom_point or geom_line accordingly.
  ...
  # lower half of full chart is beeswarm plot contrasting test scores between sending 
  # and recieving schools. Not all schools have test data available.
